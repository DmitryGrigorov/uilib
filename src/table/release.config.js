...
const config = {
  plugins: [
    ...
    [
      ...
      { pkgRoot: path.resolve("./publish-table"), npmPublish: false }
    ],
    [
    ...
    [
      ...
      {
        success: {
          path: path.resolve("./src/table/message-telegram.html")
        },
        notifications: [
          {
...
if (process.env.CI_COMMIT_REF_NAME === "dev") {
  ...
  config.plugins.push([
    "@semantic-release/git",
    {
      ...
      message:
        "chore(release): ${nextRelease.version} of table [skip ci]\n\n${nextRelease.notes}"
    }
  ]);
}
