...
import { IRadioGroupProps, TStatusRadioGroup } from "./types";

export const RadioOptionStyled = styled(RadioGroupOption)``;

interface TextProps extends Pick<IRadioGroupProps, "status"> {
  theme: ITheme;
}
...
export const RadioGroupContainerStyled = styled.div`
  display: inline-flex;
...
RadioGroupContainerStyled.defaultProps = {
  theme: LIGHT_THEME
};

export const DividerRadioGroupStyled = styled.div<
  Pick<IRadioGroupProps, "status">
>`
  margin-right: 12px;
  display: flex;
  align-items: center;
...
DividerRadioGroupStyled.defaultProps = {
  theme: LIGHT_THEME
};

export const RadioGroupStyled = styled.div<Pick<IRadioGroupProps, "direction">>`
  display: flex;
  ...
  ${({ direction }) => {
    if (direction === "column") {
      return css`
        ...
        ${RadioOptionStyled}:not(:last-child) {
          margin-bottom: 12px;
        }
      `;
    }
